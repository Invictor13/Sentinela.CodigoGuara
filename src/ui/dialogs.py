...
def show_success_dialog(root, message, folder_path, specific_path_to_copy):
    ...
    tk.Label(dialog, text="Operação Concluída com Sucesso!", font=("Segoe UI", 12, "bold"), bg=theme["window_bg"], fg=theme["success"]).pack(pady=(20, 10))
    tk.Label(dialog, text=f"A pasta de destino já foi aberta.\n{message}", font=("Segoe UI", 10), bg=theme["window_bg"], fg=theme["text_secondary"], wraplength=430).pack()

    button_frame = tk.Frame(dialog, bg=theme["window_bg"])
    ...
    copy_button = tk.Button(button_frame, text="Copiar Caminho", command=copy_path_action, font=("Segoe UI", 10, "bold"), bg=theme["button_primary_bg"], fg="white", relief=tk.FLAT, padx=10, pady=5)
